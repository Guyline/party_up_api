...
class User < ApplicationRecord
  ...
  def self.from_google_id_token(token)
    provider = "google"
    uid = token["sub"]
  ...
  def self.from_omniauth(auth)
    ...
    identity.token_expires_at = Time.at(credentials["expires_at"]).to_datetime
    identity.save!
